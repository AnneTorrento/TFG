...
for (i in 1:length(obj.list)){
  ...
  if(nrow(md) > 150){
    ...
    obj.list[[i]] <- RunUMAP(obj.list[[i]], features = features, n.neighbors = 40, verbose = F) 
...
pdf(file = "UMAP_40nn_Celltypes_group.pdf", 8, 130)
CombinePlots(plot1, ncol = 1)
dev.off()

print("CREATING PDF 2")
pdf(file = "UMAP_40nn_Celltypes_stage.pdf", 8, 130)
